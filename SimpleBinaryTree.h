...
ostream & operator << (ostream &stream, SimpleBinaryTree &tree);
...
private:
	...
	bool remove(SimpleNode* dstNode) {
		...
		if (dstNode->LChild != nullptr && dstNode->RChild != nullptr) {
			SimpleNode* leastNodeAtRChild = findLeastNodeAtRChild(dstNode);
			dstNode->key = leastNodeAtRChild->key;
			isSuccess = remove(leastNodeAtRChild);
		}
		else {
			int level = nodes.size() - 1;
			if (level > 0)	isSuccess = parentWork();
			else if (level == 0)	isSuccess = rootWork();
